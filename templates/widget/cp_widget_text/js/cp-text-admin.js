    ...
    $(document).on('widget.loaded', function(){
        $('.cp-widget-editor').AceEditor({
            ...
            onInit: function () {
                ...
                if (!this.tinymce)
                    $('<a id="content-html" class="hide-if-no-js wp-switch-editor switch-html">Text</a>')
                        .appendTo('.wp-editor-tabs');

                $('<a id="content-ace" class="hide-if-no-js wp-switch-editor switch-ace">ACE</a>')
                    .appendTo('.wp-editor-tabs')
                    .on('click.toggleAce', function () {
                        if (!self.loaded) self.load();
                    });

                $('.switch-html, .switch-tmce').on('click.destroyAce', function (e) {
            ...
            onLoaded: function () {
                ...
                $('.wp-editor-wrap').removeClass('html-active tmce-active').addClass('ace-active');
            },
            onDestroy: function (e) {
                ...
                $('.wp-editor-wrap').addClass(clicked + '-active').removeClass('ace-active');
                switch (clicked) {
                    case 'tmce':
                        this.$elem.hide();
                        break;
    ...
    AceEditor.prototype = {
        ...
        insertEditor: function () {
            $('<div id="' + this.aceId + '"></div>')
                .css({left: 0, top: 0, bottom: 0, right: 0, zIndex: 1, position: 'absolute' })
